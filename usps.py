# -*- coding: utf-8 -*-
"""
Created on Fri Sep 29 11:14:47 2017
...
if __name__ == '__main__':
    for noise_model in ['gaussian']:
        ...
        for number, index in zip(vals, locs):
            ...
            plt.imshow(image, cmap='gray', interpolation='none')
            plt.axis('off')
        plt.suptitle('First occurrence of each number, with and without noise', fontsize=17, x=0.5, y=1.05)
        plt.tight_layout()
        plt.show()
        ...
        for number in range(10):
            ...
            for i in range(10):
                ...
                plt.axis('off')
            plt.suptitle('Trained only on {}, first 10 denoised samples'.format(number), fontsize=17,  x=0.5, y=1.05)
            plt.tight_layout()
            plt.show()
        ...
        for i, n_features in enumerate(range(2, 21, 2)):
            ...
            plt.axis('off')
            plt.subplot(5, 4, 2 * i + 2)
            plt.imshow(denoised[3].reshape((16, 16)), cmap='gray', interpolation='none')
            plt.axis('off')
